...
{
	firstFreeBlock = NULL;
	struct BlockSmall* freeBlock = NULL;
	for(int i = 0; i < MAX_SMALL; ++i)
	{
		if(firstFreeBlock == NULL)
		{
			firstFreeBlock = small_tab + i;
		}
		else
		{
			*(size_t*)freeBlock = (size_t)(small_tab + i);
		}
		freeBlock = small_tab + i;
	}
...
{
	printf("Content of the %dth block : \n", blockID);
	for (unsigned int i = 0; i < SIZE_BLK_SMALL; ++i)
	{
		printf("%c/", small_tab[blockID].body[i]);
	}
	printf("\nEnd of the %dth block.\n", blockID);
...
void test_general()
{
	...
	*(ptr + 4) = -3;
	...
	printf("Just wrote int %d at the adress : %p\n", 42, (void*)(ptr + 1));



	print_block_content(0);

	for (int i = 0; i < 4; ++i)
	{
		// I can print content of the array
		read_safe_int(ptr + i);
		printf("Just read int %d at the adress : %p\n", *((int*)(ptr + i)), (void*)(ptr + i));
		read_safe_char(ptr + i);
		printf("Just read char %c at the adress : %p\n", *((char*)(ptr + i)), (void*)(ptr + i));
	}
	...
	myFree((void*)ptr);
	printf("Just freed block with adress : %p\n", (void*)ptr);
	...
	printf("Just wrote int %c at the adress : %p\n", '@', (void*)ptr2);
...
void header_test()
{
	...
	printf("Just wrote char %c at the adress : %p\n", 'E', (void*)(test2));
	...
	{
		printf("Heder containing address %p at the index %d\n", (void*)(*(size_t*)(small_tab + i)), (int)i);
	}
